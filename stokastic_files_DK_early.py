...
def adjust_stddev(df):
    """Adjust StdDev for all players in the given dataframe."""
    mask_pitcher = df['Pos'] == 'P'
    mask_hitter = df['Pos'] != 'P'
    df.loc[mask_pitcher, 'StdDev'] = df.loc[mask_pitcher, 'Fpts'] * 0.40
    df.loc[mask_hitter, 'StdDev'] = df.loc[mask_hitter, 'Fpts'] * 0.80
    return df
...
def main():
    ...
    # Adjust StdDev for all players
    projections = adjust_stddev(projections)
